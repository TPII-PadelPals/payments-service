...
from app.models.match_extended import ReserveStatus
...
async def test_notification_merchant_order_opened_leaves_payment_status_pending(
    ...
    monkeypatch.setattr(MatchesService, "update_match_player", AsyncMock())
...
async def test_notification_merchant_order_closed_sets_match_status_confirmed(
    session: AsyncSession,
    async_client: AsyncClient,
    x_api_key_header: dict[str, str],
    monkeypatch: Any,
    ...
    assert response.status_code == 200
    mock_update_match_player.assert_called_once_with(
        user_public_id, match_public_id, ReserveStatus.INSIDE
    )
